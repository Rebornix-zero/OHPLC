...
static int getportbyname(in_port_t *port, const char *service, const char *protocol) {
  printf("enter getportbyname, port %hu, service %s, protocol %s\n",(unsigned short)(*port),service,protocol);
   struct servent *se;
   int32_t tmp;   
   ...
   if (str_is_whitespace(service))                       {*port = 0;                    return -1;}
      // printf("run getservbyname\n");
  //  if ((se  = getservbyname(service, protocol)) != NULL) {printf("getservbyname succeed\n");*port = se->s_port;           return  0;}
   if ((tmp = str_to_portnum(service)) >= 0)             {printf("str_to_portnum succeed\n");*port = htons((uint16_t)tmp); return  0;}
   return -2;
}


static int getipbyname(struct in_addr *ip_addr, const char *host) {
  printf("enter getipbyname, host is %s\n",host);
   struct hostent *he;
   ...
   printf("run gethostbyname\n");
  //  if ((he = gethostbyname(host)) != NULL) {
  //   memcpy((char *)ip_addr, he->h_addr, he->h_length); 
  //   printf("gethostbyname succeed, len %d\n",he->h_length);
  //   printf("IP Address: %s\n", inet_ntoa(*ip_addr));
  //   return 0;}
