...
void *print_setter(symbol_c* symbol,
        ...
        bool external_fb = false
        ) {

  bool  is_external_fb = false;
  if (fb_symbol == NULL) {
    unsigned int vartype = analyse_variable_c::first_nonfb_vardecltype(symbol, scope_);
    ...
    if (vartype == search_var_instance_decl_c::external_vt) {
      ...
      // handle situation where we are copying a complete fb -> fb1.fb2.fb3 := fb4 (and fb3 is external!)
  ...
  wanted_variablegeneration = expression_vg;
  print_check_function(type, value, fb_value, false, external_fb);
  s4o.print(")");
  wanted_variablegeneration = expression_vg;
  return NULL;
}
    ...
    if (param_value != NULL)
      if ((param_direction == function_param_iterator_c::direction_out) ||
        ...
        if (this->is_variable_prefix_null()) {
          ...
          print_setter(param_value, param_type, param_name, NULL, symbol->fb_name, external_fb);
        }
      }
  } /* for(...) */
