char * PLC_file_name="number_socket_aarch64";
/**
...
#include <string.h>
#include "softplc_client_socket.h"//MYMODIFICATION
/*
...
void setup_pyqt_socket_interface(){}
...
timer_t PLC_timer;
char* PLC_info="Test for plc info interface \nModbus TCP Client 100.0: 192.168.157.1:502\n";//MYMODIFICATION
...
pthread_mutex_t run_mutex=PTHREAD_MUTEX_INITIALIZER;//MYMODIFICATION
...
void PLC_thread_proc(void *arg)
{
    while (!PLC_shutdown) {
        sem_wait(&Run_PLC);
		pthread_mutex_lock(&run_mutex);//MYMODIFICATION
        __run();
		pthread_mutex_unlock(&run_mutex);//MYMODIFICATION
    }
    pthread_exit(0);
}
...
{
	if(argc !=3){//MYMODIFICATION
		printf("Usage: %s <server ipv4> <server port>\n",argv[0]);//MYMODIFICATION
		exit(1);//MYMODIFICATION
	}//MYMODIFICATION


    setup_pyqt_socket_interface();
    ...
    if(  __init(argc,argv) == 0 ){
		setup_socket(argv[1], atoi(argv[2]));//MYMODIFICATION
